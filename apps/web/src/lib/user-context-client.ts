...
function makeRealUserContextClient(userId: string, getToken: () => Promise<string>): UserContextClient {
  ...
  return {
    getUserContext: () =>
      http.get<UserContextResponse>(`/api/v1/user/context?user_id=${encodeURIComponent(userId)}`),
  };
}

export function getUserContextClient(userId: string, getToken: () => Promise<string>): UserContextClient {
  if (process.env.NEXT_PUBLIC_CHAT_FIXTURES === 'true') {
    return { getUserContext: async () => FIXTURE_RESPONSE };
  }
  return makeRealUserContextClient(userId, getToken);
}
