...
- **FR-008b**: The existing `login/page.tsx` MUST hide the "Continue with Apple" button when running inside Capacitor (`Capacitor.isNativePlatform() === true`), because Apple's OAuth provider blocks WKWebView user agents with an "unsafe browser" error. The Apple button MUST remain visible in the browser web bundle. Google OAuth remains the active sign-in path on iOS. Native Sign in with Apple is out of scope for this feature.
